...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "zipline.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "zipline.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "zipline.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "zipline.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "zipline.multiplyQuantity" -}}
{{- $quantity := include "zipline.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "zipline.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "zipline.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
