...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "samba.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "samba.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "samba.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "samba.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "samba.multiplyQuantity" -}}
{{- $quantity := include "samba.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "samba.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "samba.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
