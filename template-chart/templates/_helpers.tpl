...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "xxx.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "xxx.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "xxx.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "xxx.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "xxx.multiplyQuantity" -}}
{{- $quantity := include "xxx.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "xxx.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "xxx.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
