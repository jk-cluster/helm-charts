...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "calibre-web-automated.resources" .Values.<app>.resources | nindent 12 }}
...
*/}}
{{- define "calibre-web-automated.resources" -}}
...
{{- $_ := set $requests (include "calibre-web-automated.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "calibre-web-automated.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "calibre-web-automated.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "calibre-web-automated.resourceKey" -}}
...
*/}}
{{- define "calibre-web-automated.multiplyQuantity" -}}
{{- $quantity := include "calibre-web-automated.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "calibre-web-automated.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "calibre-web-automated.formatNumber" -}}
{{- if kindIs "float64" . -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "calibre-web-automated.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
...
*/}}
{{- define "calibre-web-automated.replicas" -}}
{{- if .scaleDown -}}
0
