...
- `ci/excluded-charts.txt` — charts that cannot run in CI, each with a justification. Currently: `satisfactory` (multi-GB image + 10GB+ Steam download), `template-chart` (scaffold with placeholder image). **An excluded chart is reported as a skipped check, never as a passed one** (#231): the install-test job reads this list in its `if`, so GitHub shows `Install-test <chart>  Skipped` next to the charts that were really installed, and the detect job writes the chart's documented reason into the job summary. Until then the job ran, the script printed SKIP and exited 0, and the check list showed a green tick indistinguishable from a chart that had come up in the cluster — the failure class "Values that must not lie" is about, one level up in the pipeline. The job is skipped rather than filtered out of the matrix on purpose: an absent job is silent, and silence is what made this invisible for so long. The exclusion mechanism itself is unchanged, justification requirement included; `ci/run-install-test.sh` keeps its own SKIP branch for local runs, parsing the list identically
...
- **Documented exception — minecraft-server**: the only chart whose main image tag can be overridden, via `image.tagOverride` (#158). Only the newest Java line is maintained in the chart directory, so the JVM line stops being a property of the chart and becomes a decision made when rolling out: an older line is reached by overriding the tag rather than by installing a frozen older chart version. The override is not free of rules — the chart validates the pairing itself and fails the render, naming both values, when `minecraft.version` does not fit the Java line of the **effective** tag. `appVersion` remains the default and the only thing Renovate tracks; `image.tagOverride` is deliberately outside its reach.
