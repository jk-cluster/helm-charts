...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "apache-tika.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "apache-tika.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "apache-tika.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "apache-tika.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "apache-tika.resourceKey" -}}
...
*/}}
{{- define "apache-tika.multiplyQuantity" -}}
{{- $quantity := include "apache-tika.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "apache-tika.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "apache-tika.formatNumber" -}}
{{- if kindIs "float64" . -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "apache-tika.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
