...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "minecraft-router.resources" .Values.<app>.resources | nindent 12 }}
...
*/}}
{{- define "minecraft-router.resources" -}}
...
{{- $_ := set $requests (include "minecraft-router.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "minecraft-router.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "minecraft-router.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "minecraft-router.resourceKey" -}}
...
*/}}
{{- define "minecraft-router.multiplyQuantity" -}}
{{- $quantity := include "minecraft-router.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "minecraft-router.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "minecraft-router.formatNumber" -}}
{{- if kindIs "float64" . -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "minecraft-router.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
...
*/}}
{{- define "minecraft-router.replicas" -}}
{{- if .scaleDown -}}
0
