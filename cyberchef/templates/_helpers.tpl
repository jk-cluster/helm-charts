...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "cyberchef.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "cyberchef.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "cyberchef.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "cyberchef.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "cyberchef.multiplyQuantity" -}}
{{- $quantity := include "cyberchef.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "cyberchef.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "cyberchef.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
