...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "teamspeak-server.resources" .Values.<app>.resources | nindent 12 }}
...
*/}}
{{- define "teamspeak-server.resources" -}}
...
{{- $_ := set $requests (include "teamspeak-server.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "teamspeak-server.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "teamspeak-server.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "teamspeak-server.resourceKey" -}}
...
*/}}
{{- define "teamspeak-server.multiplyQuantity" -}}
{{- $quantity := include "teamspeak-server.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "teamspeak-server.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "teamspeak-server.formatNumber" -}}
{{- if kindIs "float64" . -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "teamspeak-server.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
...
*/}}
{{- define "teamspeak-server.replicas" -}}
{{- if .scaleDown -}}
0
