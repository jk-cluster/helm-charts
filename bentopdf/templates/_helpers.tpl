...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "bentopdf.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "bentopdf.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "bentopdf.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "bentopdf.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "bentopdf.multiplyQuantity" -}}
{{- $quantity := include "bentopdf.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "bentopdf.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
...
Usage (the values path to the workload's replicas varies per chart):
  replicas: {{ include "bentopdf.replicas" (dict "scaleDown" .Values.scaleDown "replicas" .Values.<app>.replicas) }}
...
{{- $_ := set $out $key (fromYaml (include "bentopdf.defaultedDict" (dict "defaults" $default "given" $value "path" (printf "%s.%s" $path $key)))) -}}
...
".Values.bentopdf.securityContext" can itself be erased.
*/}}
{{- define "bentopdf.subBlock" -}}
{{- $block := .block -}}
...
*/}}
{{- define "bentopdf.podSecurityContext" -}}
{{- $given := (fromYaml (include "bentopdf.subBlock" (dict "block" . "key" "pod" "path" "bentopdf.securityContext"))).value -}}
{{- $defaults := dict
      "runAsNonRoot" true
      "runAsUser" 101
      "runAsGroup" 101
      "seccompProfile" (dict "type" "RuntimeDefault") -}}
{{- include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" $given "path" "bentopdf.securityContext.pod") -}}
{{- end -}}

{{- define "bentopdf.containerSecurityContext" -}}
{{- $given := (fromYaml (include "bentopdf.subBlock" (dict "block" . "key" "container" "path" "bentopdf.securityContext"))).value -}}
{{- $defaults := dict
      "allowPrivilegeEscalation" false
      "readOnlyRootFilesystem" true
      "runAsNonRoot" true
      "capabilities" (dict "drop" (list "ALL")) -}}
{{- include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" $given "path" "bentopdf.securityContext.container") -}}
{{- end -}}

{{- define "bentopdf.livenessProbe" -}}
{{- $defaults := dict
      "httpGet" (dict "path" "/" "port" "http")
      "periodSeconds" 10
      "timeoutSeconds" 5
      "failureThreshold" 3 -}}
{{- include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" . "path" "bentopdf.livenessProbe") -}}
{{- end -}}

{{- define "bentopdf.readinessProbe" -}}
{{- $defaults := dict
      "httpGet" (dict "path" "/" "port" "http")
      "periodSeconds" 10
      "timeoutSeconds" 5
      "failureThreshold" 3 -}}
{{- include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" . "path" "bentopdf.readinessProbe") -}}
{{- end -}}

{{- define "bentopdf.startupProbe" -}}
{{- $defaults := dict
      "httpGet" (dict "path" "/" "port" "http")
      "periodSeconds" 5
      "timeoutSeconds" 5
      "failureThreshold" 30 -}}
{{- include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" . "path" "bentopdf.startupProbe") -}}
{{- end -}}
...
*/}}
{{- define "bentopdf.effectiveResources" -}}
{{- $defaults := dict
      "limitFactor" 3
      "requests" (dict "cpu" 0.1 "memory" "128Mi" "ephemeralStorage" "16Mi") -}}
{{- $merged := fromYaml (include "bentopdf.defaultedDict" (dict "defaults" $defaults "given" . "path" "bentopdf.resources")) -}}
{{- include "bentopdf.resources" $merged -}}
{{- end -}}

{{/*
The body of config.json, rendered from bentopdf.config.

The block is passed through verbatim rather than being built from named keys:
the schema of this file is upstream's (today "disabledTools" and
"editorDisabledCategories"), so a key it adds tomorrow is a values change and
not a chart change.

Unlike the probe and securityContext helpers, this one does NOT merge onto
chart defaults - there are none to merge onto, and inventing some would mean
disabling tools nobody asked to disable. An erased or missing block therefore
renders "{}", which is the same file the image itself ships. What the helper
does add is the null check plus a typed error: a scalar under bentopdf.config
would otherwise reach toJson and produce a config.json that is valid JSON,
serves with 200 and configures nothing.
*/}}
{{- define "bentopdf.configJson" -}}
{{- $config := . -}}
{{- if kindIs "invalid" $config -}}
{{- $config = dict -}}
{{- end -}}
{{- if not (kindIs "map" $config) -}}
{{- fail (printf "bentopdf.config must be a mapping or null, got %s (%v)" (kindOf $config) $config) -}}
{{- end -}}
{{- toPrettyJson $config -}}
{{- end -}}
