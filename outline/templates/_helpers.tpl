...
Usage (the values path to the resources dict varies per chart):
  resources:
    {{- include "outline.resources" .Values.<app>.resources | nindent 12 }}
...
{{- $_ := set $requests (include "outline.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- $limits := dict -}}
{{- range $key, $value := ($resources.limits | default dict) -}}
{{- if not (kindIs "invalid" $value) -}}
{{- $_ := set $limits (include "outline.resourceKey" $key) $value -}}
{{- end -}}
{{- end -}}
{{- range $key, $request := $requests -}}
{{- if and (ne $key "cpu") (not (hasKey $limits $key)) -}}
{{- $_ := set $limits $key (include "outline.multiplyQuantity" (dict "quantity" $request "factor" $factor)) -}}
{{- end -}}
{{- end -}}
{{- $out := dict -}}
...
*/}}
{{- define "outline.multiplyQuantity" -}}
{{- $quantity := include "outline.formatNumber" .quantity -}}
...
{{- else -}}
{{- printf "%s%s" (include "outline.formatNumber" $product) $suffix -}}
{{- end -}}
{{- end -}}
